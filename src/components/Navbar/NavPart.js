...
const NavPart = () => {
  ...
  return (
    <div onMouseLeave={handleMouseOver}>
      <div className="flex relative  w-full left-0 px-4 z-10 shadow-sm items-center py-4 justify-between">
        <div>
        ...
        <div>
          <ul className="flex text-md font-semibold text-gray-700 gap-4">
            ...
            <NavLink onMouseOver={handleMouseOver} to={"/blog"}>
              <li>Blog</li>
            </NavLink>

            <NavLink onMouseOver={handleMouseOver} to={"/contact"}>
              <li>Contact</li>
            </NavLink>
          </ul>
        </div>
      ...
      {dropdown === true ? <MegaMenu /> : ""}
      {Shopdropdown === true ? <ShopMenu /> : ""}
    </div>
  );
};
